...
module.exports = function TccStub(dispatch) {
    ...
    function serveExTooltipRequest(message){
        ...
        dispatch.send('C_SHOW_ITEM_TOOLTIP_EX',2,{
            type: 17,
            id: itemUid,
            unk1: 0,
            unk2: 0,
    ...
    function serveNonDbInfoRequest(message){
        ...
        dispatch.send('C_REQUEST_NONDB_ITEM_INFO', 2,{
            item: itemId,
            unk1: 0,
            button: 0
        });
    ...
    function servecAskInteractive(message){
        ...
        dispatch.send('C_ASK_INTERACTIVE', 2,{
            unk: 1,
            serverId: srvId,
            name: targetName
        });
    ...
    function serveInspect(message){
        ...
        dispatch.send('C_REQUEST_USER_PAPERDOLL_INFO', 1, {
            name: targetName
        });
    }
    ...
    function serveGuildInvite(message){
        var targetName = message.substring(message.indexOf('&name=') + 6);
        dispatch.send('C_INVITE_USER_TO_GUILD', 1 ,{
            unk1: 0,
            unk2: 0,
            name: targetName,
        });
    ...
    function serveUnblockUser(message){
        ...
        dispatch.send('C_REMOVE_BLOCKED_USER', 1, {
            name: targetName
        });
    }
    //unfriend&name=name
    function serveUnfriendUser(message){
        ...
        dispatch.send('C_DELETE_FRIEND', 1, {
            name: targetName
        });
    }
    ...
    function serveBrokerDecline(message){
        ...
        dispatch.send('C_TRADE_BROKER_REJECT_SUGGEST', 1,{
            playerId: pId,
            listing: listingId
        })
    }
    ...
    function servePartyInfoRequest(message){
        ...
        dispatch.send('C_REQUEST_PARTY_INFO', 2, {
            playerId: lfgId
        })
    }
    ...
    function serveApplyDecline(message){
        ...
        dispatch.send('C_PARTY_APPLICATION_DENIED', 1,{
            pid: playerId
        })
    }
    ...
    function servePowerChange(message){
        ...
        dispatch.send('C_CHANGE_PARTY_MEMBER_AUTHORITY', 1,{
            serverId: sId,
            playerId: pId,
            canInvite: p
        })
    ...
    function serveLeaderChange(message){
        ...
        dispatch.send('C_CHANGE_PARTY_MANAGER',2,{
            serverId: sId,
            playerId: pId
        })
    }
    ...
    function serveKickMember(message){
        ...
        dispatch.send('C_BAN_PARTY_MEMBER',1,{
            serverId: sId,
            playerId: pId
        })
    }
	...
	function serveLfgPageRequest(message){
		...
		dispatch.send('C_REQUEST_PARTY_MATCH_INFO_PAGE',1,{
			page: p,
			unk1: 3,
			unk2: 0
		})
	...
	function serveRegisterLfg(message){
		...
		dispatch.send('C_REGISTER_PARTY_INFO', 1, {
			isRaid: raid,
			message: msg
		});
	}
	...
	function serveRemoveLfg(message){
		dispatch.send('C_UNREGISTER_PARTY_INFO', 1, {
			unk1: 20,
			minLevel: 1,
			maxLevel: 65,
			unk3: 3,
	...
	function servePublicizeLfg(message){
		dispatch.send('C_REQUEST_PARTY_MATCH_LINK', 1, { });
	}
	//reset_instance
	function serveResetRequest(message){
		dispatch.send('C_RESET_ALL_DUNGEON', 1, { });
	}
	//disband_group
	function serveDisbandRequest(message){
		dispatch.send('C_DISMISS_PARTY', 1, { });
	}
	//leave_party
	...
	function serveRequestCandidates(message){
		dispatch.send('C_REQUEST_CANDIDATE_LIST', 1, { });
	}
	//lfg_request_list&minlvl=minlvl&maxlvl=maxlvl
	function serveRequestLfgList(message){
		dispatch.send("C_PARTY_MATCH_WINDOW_CLOSED", 1, {});
		...
		if(min > max) min = max;
		if(min < 1) min = 1;
		dispatch.send("C_REQUEST_PARTY_MATCH_INFO", 1, {
			unk1: 0,
			minlvl: min,
			maxlvl: max,
			unk2: 3,
